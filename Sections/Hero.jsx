        ...
        <div className={`relative w-full z-0 mt-[0.5rem] ${playfair.className}`} style={{ height: `${height}vh` }}>
            ...
            {/* <div className="flex flex-col items-center "> */}
                <div className={`absolute inset-0 flex flex-col  justify-center   text-white px-4 ${playfair.className}`}>
                    <h1 className={` md:text-4xl text-white inline-flex gap-2 font-bold uppercase tracking-wide  text-transparent bg-clip-text sm:text-5xl lg:text-5xl ${className}`}>{title}</h1>
                    <p className={` md:text-6xl lg:text-6xl itallic text-white inline-flex gap-2 font-bold uppercase tracking-wide   text-transparent bg-clip-text sm:text-[1.7rem]   mt-2 ${className2}`}>{subBody}</p>
                </div>
            {/* </div> */}
        </div>
    );
